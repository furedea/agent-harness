...
`agent-harness install --enable-herdr` runs `herdr integration install` for Claude and Codex in a
temporary home, then merges the generated hooks and Codex feature settings into the harness output.
Use `--herdr-bin PATH` when `herdr` is not on `PATH`.
