...
- Prefer one coherent VCS unit per applicable TSDD cycle. Behavior-preserving work may start from a Green baseline. If the task is too small for multiple cycles, one commit is enough.
...
3. Classify the work under TSDD: use Red -> Green -> Refactor for a real contract gap; otherwise establish a Green baseline and run the relevant verification gates.
