...
let
  ...
  agentsMd = if cfg.agentsMd == null then "${profileSource}/agents/AGENTS.md" else cfg.agentsMd;
  commandPermissions =
    if cfg.commandPermissions == null then
      "${profileSource}/agents/command_permissions.json"
    else
  ...
  composedSource = pkgs.runCommand "agent-harness-${cfg.profile}-source" { } ''
    ...
    chmod -R u+w "$out"
    install -m 0644 ${lib.escapeShellArg (toString agentsMd)} "$out/agents/AGENTS.md"
    install -m 0644 ${lib.escapeShellArg (toString commandPermissions)} \
      "$out/agents/command_permissions.json"
...
in
{
  options.programs.agent-harness = {
    ...
    source = lib.mkOption {
      type = lib.types.path;
      default = self;
      description = "Source tree containing agent-harness profiles.";
    };
